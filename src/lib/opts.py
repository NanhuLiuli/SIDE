...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('task', default='stereo',
                             help='stereo | ddd ')
    ...
    self.parser.add_argument('--trainval', action='store_true',
                             help='include validation in training and '
                                  'test on test set')
    self.parser.add_argument('--flip_train', action='store_true',
    ...
    self.parser.add_argument('--no_color_aug', action='store_true',
                             help='not use the color augmenation '
                                  'from CornerNet')
    # ddd
    self.parser.add_argument('--aug_ddd', type=float, default=0.35,
    ...
    self.parser.add_argument('--dim_orien_weight', type=float, default=1,
                             help='loss weight for 3d bounding box size.')
    self.parser.add_argument('--dim_weight', type=float, default=1,
                             help='loss weight for 3d bounding box size.')
    self.parser.add_argument('--orien_weight', type=float, default=1,
                             help='loss weight for 3d bounding box size.')
    self.parser.add_argument('--kept_weight', type=float, default=1,
                             help='loss weight for keypoint.')
    self.parser.add_argument('--depth_weight', type=float, default=1,
                             help='loss weight for depth.')
    self.parser.add_argument('--peak_thresh', type=float, default=0.2)
    self.parser.add_argument('--uncert', action='store_true', 
                             help='use uncertainty to weigh losses')
    self.parser.add_argument('--not_cost_volume', action='store_true',
                             help='not use 3d conv')
    self.parser.add_argument('--wh_scale', type=float, default=1,
                             help='wh scale to build cost volume')
  ...
  def parse(self, args=''):
    ...
    opt.cost_volume = not opt.not_cost_volume
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    opt.dim_exp = dataset.dim_exp
    opt.grid = 28
    opt.lossWeight = [opt.hm_weight, opt.wh_weight, opt.off_weight, \
                          opt.depth_weight, opt.dim_weight, opt.orien_weight, opt.kept_weight]
    ...
    if opt.task == 'stereo':
      ...
      opt.heads = {'hm': opt.num_classes, 'dim': 3, 'orien':2, 'kept_type': 6*opt.grid}
      if opt.reg_bbox:
        opt.heads.update(
          {'wh': 3})
      if opt.reg_offset:
