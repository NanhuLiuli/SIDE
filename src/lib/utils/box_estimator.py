...
def solve_x_y_z_theta_from_kpt(im_shape, calib, alpha, dim, box_left, box_right, depth, kpts):
    ...
    def j_kpt(states):
        ...
        dx = float(dul_dx + dur_dx + duk_dx + dvb_dx + dvt_dx + dul_r_dx + dur_r_dx + dalpha_dx)
        dy = float(dul_dy + dur_dy + duk_dy + dvb_dy + dvt_dy + dul_r_dy + dur_r_dy + dalpha_dy)
        dz = float(dul_dz + dur_dz + duk_dz + dvb_dz + dvt_dz + dul_r_dz + dur_r_dz + dalpha_dz)
        dth = float(dul_dth + dur_dth + duk_dth + dvb_dth + dvt_dth + dul_r_dth + dur_r_dth + dalpha_dth)
    ...
    init_z = float(depth)
    init_x = float(init_z*(left_u+right_u)/2.0)
    init_y = float(init_z*(bottom_v+top_v)/2.0 + h/2.0)
    init_theta = float(alpha + m.pi/2 - m.atan2(-init_x, init_z))

    inp = np.array([init_x, init_y, init_z, init_theta], dtype=np.float32)
    
    res = minimize(f_kpt, inp, method ='Newton-CG', jac=j_kpt, options={'disp': False}) 
