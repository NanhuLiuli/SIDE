...
def align_parallel(calib, opt, im_left, im_right, \
		  			box_left, borders, poses):
	''' Dense alignment for multiple objects in parallel for depth enumeration

	Inputs:
		im_left, im_right: 1 x 3 x H x W
		box_left: rois x 4 in origin image
		borders: rois x 2 (left_border, right_border in origin image)
		poses: rois x 7 (x, y, z, w, h, l, theta)
	Return:
		solve_status: 1 denotes success, 0 denotes faild due to no valid pixel (rois x 1)
		best_dis: aligned disparity in origin image (rois x 1)
	...
	im_left = (im_left.astype(np.float32) / 255.)
	im_left = (im_left - mean) / std
	im_left = im_left.transpose(2, 0, 1)[np.newaxis, ...]

	im_right = (im_right.astype(np.float32) / 255.)
	im_right = (im_right - mean) / std
	im_right = im_right.transpose(2, 0, 1)[np.newaxis, ...]
	im_left, im_right = torch.from_numpy(im_left), torch.from_numpy(im_right)
