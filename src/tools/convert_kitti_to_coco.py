...
for SPLIT in SPLITS:
  ...
  for split in splits:
    ...
    for line in image_set:
      ...
      for ann_ind, txt in enumerate(anns):
        ...
        if DEBUG and tmp[0] != 'DontCare':
          ...
          box_2d = project_to_image(box_3d, np.array(calib[2]))
          # print('box_2d', box_2d)
          image = draw_box_3d(image, np.array(box_2d, dtype=np.int))
          x = (bbox[0] + bbox[2]) / 2
          '''
          ...
          pt_2d = np.array([(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2],
                            dtype=np.float32)
          pt_3d = unproject_2d_to_3d(pt_2d, depth, np.array(calib[2]))
          pt_3d[1] += dim[0] / 2
          print('pt_3d', pt_3d)
    ...
    out_path = '{}/annotations_3d/kitti_{}_{}.json'.format(DATA_PATH, SPLIT, split)
